...
const RULER_REFERENCE_MM = 50;
const ALGERIAN_DINAR_DIAMETER_MM = 26.5;
...
function App() {
  ...
  const [calibrationMethod, setCalibrationMethod] =
    useState<CalibrationMethod>("card");
  const [pixelsPerMM, setPixelsPerMM] = useState(0);

  const t = translations[language];
  const isRTL = language === "ar";

  // Charger la calibration depuis le localStorage au démarrage
  useEffect(() => {
  ...
  // Envoyer la hauteur au parent (iframe auto-resize)
  useEffect(() => {
    const sendHeight = () => {
